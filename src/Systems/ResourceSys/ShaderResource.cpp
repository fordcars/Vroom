...
ShaderResource::ShaderResource(const std::string& name, const std::filesystem::path& vertexPath,
...
GLint ShaderResource::getUniformBlock(std::size_t uniformBlockNameIndex) const {
    if (uniformBlockNameIndex >= mUniformBlockLocations.size()) {
        Log::error() << "Uniform block name index " << uniformBlockNameIndex << " out of bounds.";
        return -1;
    }
...
// Static
GLuint ShaderResource::compileShader(const std::filesystem::path& shaderPath, const std::string& shaderSource,
                                     GLenum type) {
...
// Static
GLuint ShaderResource::linkShaderProgram(const std::string& shaderProgramName, GLuint vertexShader,
...
// Static
std::string ShaderResource::getGLShaderDebugLog(GLuint object, PFNGLGETSHADERIVPROC glGet_iv,
    ...
    if (logLength)
        glGet_InfoLog(object, logLength, nullptr, log.data());
...
void ShaderResource::registerUniforms() {
    ...
    for (int i = 0; i < count; i++) {
        ...
        Log::debug() << "Uniform block '" << uniformNameBuffer << "' bound to binding point " << bindingPointCounter;
...
void ShaderResource::registerUniform(const std::string& uniformName) {
    ...
    if (uniformNameIndex.has_value()) {
        if (mUniformLocations[uniformNameIndex.value()] != -1) {
            Log::warn() << "Uniform '" << uniformName << "' already registered for shader '" << mName << "'.";
            return;
        }
...
void ShaderResource::registerUniformBlock(const std::string& uniformBlockName, GLuint bindingPoint) {
    auto uniformBlockNameIndex = Constants::UniformBlockName::runtimeGet(uniformBlockName);
    if (uniformBlockNameIndex.has_value()) {
        if (mUniformBlockLocations[uniformBlockNameIndex.value()] != -1) {
            Log::warn() << "Uniform block '" << uniformBlockName << "' already registered for shader '" << mName
                        << "'.";
            return;
        }
        Log::debug() << "Registering uniform block '" << uniformBlockName << "' in shader'" << mName
