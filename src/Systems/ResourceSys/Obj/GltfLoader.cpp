...
namespace {
template <typename T>
void extractAttribute(const tinygltf::Primitive& primitive, const tinygltf::Model& model, const std::string& attribute,
                      std::vector<T>& output) {
    ...
    if constexpr (std::is_same<T, glm::vec3>::value) {
        ...
        if (accessor.componentType == TINYGLTF_COMPONENT_TYPE_UNSIGNED_BYTE) {
            ...
            for (size_t i = 0; i < accessor.count; i++) {
                output[i] = glm::uvec4(data[i * 4 + 0], data[i * 4 + 1], data[i * 4 + 2], data[i * 4 + 3]);
            }
...
void GltfLoader::loadMaterials(ObjResource& resource, const tinygltf::Model& model) {
    ...
    for (const tinygltf::Material& gltfMaterial : model.materials) {
        ...
        material.sheen = 0.0f; // Not supported in GLTF core, but could be added via extensions.
...
void GltfLoader::loadImages(ObjResource& resource, const tinygltf::Model& model) {
    for (const tinygltf::Image& gltfImage : model.images) {
        if (gltfImage.image.empty()) {
            ...
            resource.objImages.emplace_back(ObjImage::create(gltfImage.name, 0, 0, 0)); // Empty image
            continue;
        }
        ...
        // Load image data
        Log::debug() << "Loading image '" << gltfImage.name << "' with dimensions " << gltfImage.width << "x"
                     << gltfImage.height << " and " << gltfImage.component << " components.";

        GLint format = (gltfImage.component == 3) ? GL_RGB : GL_RGBA;
        glTexImage2D(GL_TEXTURE_2D, 0, format, gltfImage.width, gltfImage.height, 0, format, GL_UNSIGNED_BYTE,
        ...
        // Create ObjImage
        auto objImage = ObjImage::create(gltfImage.name, texId, gltfImage.width, gltfImage.height);
...
void GltfLoader::loadTextures(ObjResource& resource, const tinygltf::Model& model) {
    ...
    for (const tinygltf::Texture& gltfTexture : model.textures) {
        if (gltfTexture.source < 0) {
            ...
            resource.objTextures.emplace_back(ObjTexture::create(gltfTexture.name, emptyImage, 0)); // Empty texture
            continue;
        }

        if (gltfTexture.source >= static_cast<int>(resource.objImages.size())) {
            Log::warn() << "Texture '" << gltfTexture.name << "' has an invalid image source.";
            resource.objTextures.emplace_back(ObjTexture::create(gltfTexture.name, emptyImage, 0)); // Empty texture
            continue;
        }
        ...
        GLint minFilter = sampler.minFilter != -1 ? sampler.minFilter : GL_LINEAR_MIPMAP_LINEAR;
        ...
        // Create ObjTexture
        auto objTexture = ObjTexture::create(gltfTexture.name, resource.objImages[gltfTexture.source], samplerId);
...
void GltfLoader::loadMeshes(ObjResource& resource, const tinygltf::Model& model) {
    ...
    while (!nodeStack.empty()) {
        ...
        glm::mat4 localTransform = glm::translate(glm::mat4(1.0f), translation) * glm::mat4_cast(rotation) *
        ...
        for (int childIndex : node.children) {
            nodeStack.push({childIndex, currentSkin, transform}); // Push child node with inherited skin
        }
    }
...
void GltfLoader::loadPrimitives(ObjResource& resource, std::vector<ObjResource::Vertex>& outVertices,
    ...
    if (resource.animationContainer) {
        ...
        Log::debug() << "Loading animated mesh '" << mesh.name << "' with " << mesh.primitives.size()
                     << " primitive(s).";
    } else {
        Log::debug() << "Loading mesh '" << mesh.name << "' with " << mesh.primitives.size() << " primitive(s).";
    }
    ...
    if (resource.animationContainer && gltfSkinIndex >= 0) {
        ...
        if (!skin) {
            Log::warn() << "Failed to find skin with index " << gltfSkinIndex << " for mesh '" << mesh.name << "'.";
        }
    }
    ...
    for (size_t i = 0; i < mesh.primitives.size(); i++) {
        ...
        for (size_t i = 0; i < vertexCount; i++) {
            outVertices.emplace_back(positions[i], normals[i], texcoords[i], materialId, joints[i], weights[i]);
        }
        ...
        auto objMesh = ObjMesh::create(resource, mesh.name + "_p" + std::to_string(i), std::move(primitiveIndices));
...
void GltfLoader::setMeshTextures(ObjResource& resource, ObjMesh& mesh, const tinygltf::Model& model,
                                 const tinygltf::Primitive& primitive) {
    auto getTexture = [&resource](const tinygltf::TextureInfo& texInfo, const std::string& type) -> ObjTexture::Ptr {
        ...
        if (texInfo.index < 0)
            return nullptr;
        Log::debug() << "Using image " << resource.objTextures[texInfo.index]->image->name << " for mesh " << type
                     << " texture.";
        return resource.objTextures[texInfo.index];
    };

    auto getNormalTexture = [&resource](const tinygltf::NormalTextureInfo& texInfo) -> ObjTexture::Ptr {
        if (texInfo.texCoord > 0) {
            Log::warn() << "Normal texture coordinate sets other than 0 are not supported.";
        }
    ...
    mesh.baseColorTexture = getTexture(gltfMaterial.pbrMetallicRoughness.baseColorTexture, "base color");
    mesh.normalTexture = getNormalTexture(gltfMaterial.normalTexture);
    mesh.metallicRoughnessTexture =
        getTexture(gltfMaterial.pbrMetallicRoughness.metallicRoughnessTexture, "metallic roughness");
