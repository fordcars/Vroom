...
void Animation::loadAnimationData(AnimationContainer& container, const tinygltf::Model& model,
                                  const tinygltf::Animation& animation) {
    ...
    for (size_t i = 0; i < animation.channels.size(); i++) {
        ...
        animChannel.targetPath = channel.target_path == "translation" ? TargetPath::Translation
        ...
        const auto* timeData =
            reinterpret_cast<const float*>(&inputBuffer.data[inputView.byteOffset + input.byteOffset]);
        ...
        const unsigned char* dataPtr = outputBuffer.data.data() + outputView.byteOffset + output.byteOffset;
        size_t stride = (outputView.byteStride == 0)
        ...
        for (size_t j = 0; j < output.count; j++) {
            ...
            const float* valueData = reinterpret_cast<const float*>(dataPtr + j * stride);
