...
void AnimationContainer::loadNodes(const tinygltf::Model& model, int parentNodeIndex) {
    ...
    std::stack<std::pair<int, AnimationNode*>> stack; // Input node index, our parent node
    stack.push({parentNodeIndex, nullptr});

    while (!stack.empty()) {
        auto [nodeIndex, parentNode] = stack.top();
        stack.pop();
        if (mVisitedInputNodes.contains(nodeIndex))
            continue;
        ...
        newNode->scale = node.scale.empty() ? glm::vec3(1.0f) : glm::vec3(node.scale[0], node.scale[1], node.scale[2]);
...
void AnimationContainer::loadAnimations(const tinygltf::Model& model) {
    ...
    for (const auto& anim : model.animations) {
        std::optional<std::size_t> nameIndex = Constants::AnimationName::runtimeGet(anim.name);
        if (nameIndex.has_value()) {
            Animation::Ptr animation = Animation::create(*this, model, anim);
            mAnimations[nameIndex.value()] = animation;
        } else {
            Log::debug() << "Animation name '" << anim.name << "' not recognized, skipping.";
            continue;
        }
    }
}