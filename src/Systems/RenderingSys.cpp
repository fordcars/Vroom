...
void RenderingSys::setPostProcessShader(ShaderResource::CPtr shader) { mPostProcessShader = std::move(shader); }

void RenderingSys::addDebugShape(const std::vector<glm::vec3>& points, const std::vector<glm::vec3>& colors,
                                 GLenum drawMode) {
    // Resize colors if not same size
    std::vector<glm::vec3> resizedColors(points.size(), glm::vec3(0.0f, 1.0f, 0.0f)); // Default to green
...
void RenderingSys::initDeferredRendering() {
    static constexpr std::pair<GBufferTexture, GLint> TEXTURE_FORMATS[] = {
        {GBufferTexture::Position, GL_RGB32F}, {GBufferTexture::Normal, GL_RGB32F}, {GBufferTexture::Albedo, GL_RGB8},
        {GBufferTexture::Metallic, GL_RED},    {GBufferTexture::Roughness, GL_RED},
    };
    ...
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT, mScreenSize.x, mScreenSize.y);
    ...
    for (size_t i = 0; i < GBufferTexture::COUNT; ++i) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, format, mScreenSize.x, mScreenSize.y, 0, GL_RED, GL_FLOAT, nullptr);
        glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + texture, mDeferredTextures[texture], 0);
    ...
    GLenum error = glGetError();
    if (error != GL_NO_ERROR) {
        Log::glError(error) << "Encountered an OpenGL error while initializing deferred rendering!";
    }
}
...
void RenderingSys::initPostProcessRendering() {
    ...
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT, mScreenSize.x, mScreenSize.y);
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, mPostProcessDepthBuffer);

    // Generate texture
    glGenTextures(1, &mPostProcessTexture);
    glBindTexture(GL_TEXTURE_2D, mPostProcessTexture);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, mScreenSize.x, mScreenSize.y, 0, GL_RED, GL_FLOAT, nullptr);
    ...
    GLenum error = glGetError();
    if (error != GL_NO_ERROR) {
        Log::glError(error) << "Encountered an OpenGL error while initializing post-process rendering!";
    }
}

void RenderingSys::renderRenderable(const glm::mat4& viewMatrix, // NOLINT(readability-make-member-function-const)
                                    const glm::mat4& projectionMatrix, const PositionComp& position,
    ...
    auto setTexture = [](std::size_t samplerUniformLocation, std::size_t hasSamplerUniformLocation,
                         const ObjTexture* texture, GLint textureUnit) {
        if (texture) {
    ...
    bool isSkinnedShader = (skinTransformUnformBlock = renderable.shader->getUniformBlock(
                           ...
                           (isSkinnedUniform = renderable.shader->getUniform(UniformName::get<"isSkinned">())) != -1;
    ...
    if (materialsBlock != -1) {
        glBindBufferBase(GL_UNIFORM_BUFFER, materialsBlock, renderable.objectResource->materialUniformBuffer.getId());
    }
    ...
    for (const auto& mesh : renderable.objectResource->objMeshes) {
        ...
        glUniformMatrix4fv(shader.getUniform(UniformName::get<"MVP">()), 1, GL_FALSE, &MVP[0][0]);
        glUniformMatrix4fv(shader.getUniform(UniformName::get<"modelMatrix">()), 1, GL_FALSE, &modelMatrix[0][0]);
        glUniformMatrix4fv(shader.getUniform(UniformName::get<"viewMatrix">()), 1, GL_FALSE, &viewMatrix[0][0]);
        glUniformMatrix4fv(shader.getUniform(UniformName::get<"projectionMatrix">()), 1, GL_FALSE,
                           &projectionMatrix[0][0]);
        glUniformMatrix4fv(shader.getUniform(UniformName::get<"normalMatrix">()), 1, GL_FALSE, &normalMatrix[0][0]);

        // Set textures if present
        int textureUnit = 0;
        setTexture(shader.getUniform(UniformName::get<"baseColorTex">()),
                   shader.getUniform(UniformName::get<"hasBaseColorTex">()), mesh->baseColorTexture.get(),
                   textureUnit++);
        setTexture(shader.getUniform(UniformName::get<"normalTex">()),
                   shader.getUniform(UniformName::get<"hasNormalTex">()), mesh->normalTexture.get(), textureUnit++);
        ...
        setTexture(shader.getUniform(UniformName::get<"emissiveTex">()),
                   shader.getUniform(UniformName::get<"hasEmissiveTex">()), mesh->emissiveTexture.get(), textureUnit++);

        glUniform1f(shader.getUniform(UniformName::get<"normalScale">()), mesh->normalScale);

        if (isSkinnedShader) {
            if (mesh->skin) {
                glUniform1i(isSkinnedUniform, 1);
                glBindBufferBase(GL_UNIFORM_BUFFER, skinTransformUnformBlock, mesh->skin->getTransformBuffer().getId());
            } else {
...
void RenderingSys::renderLight(const glm::mat4& viewMatrix, const PositionComp& position, const LightComp& light) {
    if (!light.shader) {
        return;
    }
    ...
    // Set uniforms
    glUniformMatrix4fv(shader.getUniform(UniformName::get<"viewMatrix">()), 1, GL_FALSE, &viewMatrix[0][0]);
    glUniform3f(shader.getUniform(UniformName::get<"lightPos_worldspace">()), position.coords.x, position.coords.y,
                position.coords.z);
    glUniform3f(shader.getUniform(UniformName::get<"lightDiffuseColor">()), light.diffuse.r, light.diffuse.g,
                light.diffuse.b);
    glUniform3f(shader.getUniform(UniformName::get<"lightSpecularColor">()), light.specular.r, light.specular.g,
                light.specular.b);
    ...
    // Positions (vec3)
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, stride, (void*)offsetof(LightComp::Vertex, position)); // NOLINT

    // UVs (vec2)
    glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, stride, (void*)offsetof(LightComp::Vertex, texcoord)); // NOLINT
...
void RenderingSys::renderPostProcessing() {
    ...
    // Note at this point, we are probably drawing a simple, full screen quad
    // Positions (vec3)
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, stride, (void*)offsetof(Vertex, position)); // NOLINT

    // UVs (vec2)
    glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, stride, (void*)offsetof(Vertex, texcoord)); // NOLINT
...
void RenderingSys::renderDebugShape(const ShaderResource& shader, const glm::mat4& viewMatrix,
                                    const glm::mat4& projectionMatrix, const DebugShape& shape, GLenum drawMode) {
...
void RenderingSys::cloneDepthBuffer(GLuint source, GLuint dest) { // NOLINT(readability-make-member-function-const)
    ...
    glBlitFramebuffer(0, 0, mScreenSize.x, mScreenSize.y, 0, 0, mScreenSize.x, mScreenSize.y, GL_DEPTH_BUFFER_BIT,
                      GL_NEAREST);
}
...
glm::mat4 RenderingSys::getProjectionMatrix(const CameraEntity& camera) {
    ...
    return glm::perspective(vertFOX, info.aspectRatio, info.nearClippingPlane, info.farClippingPlane);
}