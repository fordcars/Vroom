...
void PhysicsSys::update(float deltaTime) {
    ...
    for (const auto& [position, physics, frictionComp, gravityComp] :
         EntityFilter<PositionComp, NullPhysicsComp, std::optional<FrictionComp>, std::optional<GravityComp>>()) {
    ...
    for (const auto& [position, physics, renderable, frictionComp, gravityComp] :
         EntityFilter<PositionComp, BoxPhysicsComp, RenderableComp, std::optional<FrictionComp>,
                      std::optional<GravityComp>>()) {
        // Update bounding box
        auto [otherMin, otherMax] = renderable.objectResource->boundingBox->getWorldspaceAABB(
    ...
    for (auto& [position, physics, frictionComp, gravityComp] :
         EntityFilter<PositionComp, SpherePhysicsComp, std::optional<FrictionComp>, std::optional<GravityComp>>()) {
        handleSphereEntityCollision(position, physics, frictionComp, gravityComp, deltaTime);
    }
}
...
void PhysicsSys::updatePhysics(PhysicsComp& physics,
...
void PhysicsSys::handleSphereEntityCollision(PositionComp& position, SpherePhysicsComp& physics,
                                             const std::optional<std::reference_wrapper<FrictionComp>>& frictionComp,
    ...
    glm::vec3 newPosition = position.coords + physics.positionOffset + physics.velocity * deltaTime;
    ...
    for (const auto& [otherPhysics] : EntityFilter<BoxPhysicsComp>()) {
        ...
        bool colliding = newPosition.x >= expandedMin.x && newPosition.x <= expandedMax.x &&
        ...
        glm::vec3 penetrationDepth =
            glm::vec3(std::min(std::abs(newPosition.x - expandedMin.x), std::abs(newPosition.x - expandedMax.x)),
                      std::min(std::abs(newPosition.y - expandedMin.y), std::abs(newPosition.y - expandedMax.y)),
                      std::min(std::abs(newPosition.z - expandedMin.z), std::abs(newPosition.z - expandedMax.z)));
...
void PhysicsSys::drawCollisionShapes() {
    auto drawBox = [this](const glm::vec3& minCorner, const glm::vec3& maxCorner, const glm::mat4& transform,
        ...
        RenderingSys::get().addDebugShape(vertices, std::vector<glm::vec3>(vertices.size(), color), GL_LINES);
    };
    ...
    for (const auto& [position, renderable, physics] : boxEntites) {
        ...
        drawBox(obb->minCorner, obb->maxCorner, position.getTransform(), {0.53f, 0.53f, 1.0f});
        drawBox(physics.minCorner, physics.maxCorner, glm::mat4(1.0f),
                {1.0f, physics.currentCollision.colliding ? 1.0f : 0.0f, 0.0f});

        // Draw velocity vectors
        RenderingSys::get().addDebugShape({physics.minCorner, physics.minCornerVelocity + physics.minCorner},
                                          std::vector<glm::vec3>(2, {0, 1, 0.5}), GL_LINES);
        RenderingSys::get().addDebugShape({physics.maxCorner, physics.maxCornerVelocity + physics.maxCorner},
                                          std::vector<glm::vec3>(2, {0, 1, 0.5}), GL_LINES);
    }
    ...
    for (const auto& [position, renderable, physics] : sphereEntities) {
        const auto& debugPoint = Utils::generateSphere(physics.radius, position.coords + physics.positionOffset);

        for (const auto& circle : debugPoint) {
            RenderingSys::get().addDebugShape(
                circle,
                std::vector<glm::vec3>(circle.size(), {1.0f, physics.currentCollision.colliding ? 1.0f : 0.0f, 0.0f}),
                GL_LINE_LOOP);
        }
    }
}
